...
引き続き、Sepolia Test Network 上にコントラクトをデプロイするので、ユーザーに送るのは偽の ETH になります。
...
2\. ローカル環境で Web アプリケーションを開き、`wave`を送る。
...
あなたの Etherscan のアドレスを`#ethereum`に投稿してあなたの成功をコミュニティで祝いましょう 😊
